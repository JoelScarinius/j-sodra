...
def _corner_side_payload(
    ...
    return {
        **summary,
        ...
        "top_takers_by_first_shot_xg": takers.get(
            "top_takers_by_first_shot_xg", []
        ),
...
def _corner_plot_ref(settings, path, plot_name: str, team_name: str, opponent_name: str | None):
    ref = build_ref(settings, path)
    if not path:
        return ref

    subject_name = team_name if plot_name.startswith("team_") else (opponent_name or "Next opponent")
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Fetch and publish J-Sodra football analytics."
    )
    parser.add_argument(
        "--force-refresh",
        action="store_true",
...
def main(argv=None) -> int:
    ...
    if settings.upload_to_supabase_storage:
        ...
        if deleted:
            print(f"Removed {len(deleted)} obsolete corner-origin heatmap objects from storage.")
