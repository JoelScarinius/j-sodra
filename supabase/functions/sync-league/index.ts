...
function expectedPointsFromXg(_xgFor: number | null, _xgAgainst: number | null) {
  if (_xgFor === null || _xgAgainst === null) {
    return null;
  }

  const lambdaFor = Math.max(0, Number(_xgFor));
  const lambdaAgainst = Math.max(0, Number(_xgAgainst));
  const maxGoals = 10;
  ...
  for (let homeGoals = 0; homeGoals <= maxGoals; homeGoals += 1) {
    const homeProbability = poisson(lambdaFor, homeGoals);
    if (homeProbability === 0) continue;
    for (let awayGoals = 0; awayGoals <= maxGoals; awayGoals += 1) {
      const awayProbability = poisson(lambdaAgainst, awayGoals);
      if (awayProbability === 0) continue;

      const outcomeProbability = homeProbability * awayProbability;
      if (homeGoals > awayGoals) {
        expectedPoints += 3 * outcomeProbability;
      } else if (homeGoals === awayGoals) {
        expectedPoints += outcomeProbability;
      }
    }
  }

  return Math.round(expectedPoints * 1000) / 1000;
}
...
async function wyscoutFetch(endpoint: string, params: Record<string, unknown> = {}) {
  ...
  let lastMessage = "Unknown Wyscout error";
  const attempts = Math.max(1, wyscoutMaxRetries + 1);

  for (let attempt = 0; attempt < attempts; attempt += 1) {
    if (wyscoutRequestDelayMs > 0) {
      await sleep(wyscoutRequestDelayMs);
    }
    ...
    lastMessage = payload?.error?.message
      ...
      || payload?.message
      || text
      || `Wyscout request failed with ${response.status}`;

    const shouldRetry = response.status === 429 || isRateLimitMessage(String(lastMessage));
    if (!shouldRetry || attempt >= attempts - 1) {
      throw new Error(String(lastMessage));
    }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    let selectedSummaries = requestedSeasonIds.length
    ...
    if (maxMatches > 0) {
      selectedSummaries = [...selectedSummaries]
        .sort((left, right) => Date.parse(right.kickoffAt ?? "") - Date.parse(left.kickoffAt ?? ""))
        .slice(0, maxMatches);
    }

    if (!selectedSummaries.length) {
      throw new Error("No matches found for the selected league, season, team, and max_matches scope");
    }
